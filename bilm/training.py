...
class LanguageModel(object):
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            embedding_addr = './data/callreason-char-300d'
            f = open(embedding_addr,'r')
            wordvecs = f.readlines()
            f.close()
            vocab_size, vocab_dim = list(map(int,wordvecs[0].strip().split()))

            tmp_embed = np.zeros((vocab_size+3, vocab_dim),dtype=DTYPE)
            # <S> </S> <UNK>
            for idx,wd in enumerate(wordvecs[1:]):
                tmp_embed[3+idx,:] = np.array(list(map(float,wd.strip().split()[1:])),dtype=DTYPE)
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if (batch_no % 1250 == 0) or (batch_no == n_batches_total):
                ...
                import h5py
                with h5py.File('vocab_embedding.word.hdf5','w') as fout:
